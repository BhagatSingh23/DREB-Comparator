...
public class SlidingWindowLogProperties {
    @Property
    void acceptedNeverExceedsLimitInSlidingWindow(@ForAll("requestSequences") List<RateLimitRequest> requests) {
        RateLimiterConfig config = RateLimiterConfig.builder().maxRequests(10).windowSizeMs(1000).build();
        RateLimiter limiter = new com.ratelimiter.algorithms.SlidingWindowLog(config);
