...
public class LeakyBucketProperties {
    @Property
    void allowanceNeverGoesNegative(@ForAll("requestSequences") List<RateLimitRequest> requests) {
        RateLimiterConfig config = RateLimiterConfig.builder().bucketCapacity(10).refillRate(1).build();
        RateLimiter limiter = new com.ratelimiter.algorithms.LeakyBucket(config);
