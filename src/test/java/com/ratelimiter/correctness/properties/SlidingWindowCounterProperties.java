...
public class SlidingWindowCounterProperties {
    @Property
    void actualWithinBoundedApproximation(@ForAll("requestSequences") List<RateLimitRequest> requests) {
        RateLimiterConfig config = RateLimiterConfig.builder().maxRequests(10).windowSizeMs(1000).build();
        RateLimiter limiter = new com.ratelimiter.algorithms.SlidingWindowCounter(config);
